...
describe 'Favorites Endpoints' do
  ...
  describe 'Get all saved favorites for a user' do
    ...
    describe 'sad path' do
      it 'returns and empty array if user has no favorites' do
        user = create(:user)
        create_list(:favorite, 5)
        ...
        expect(favorites_data).to be_an(Array)
        expect(favorites_data).to be_empty

        expect(favorites_response).to eq({ data: [] })
      end

      it 'returns a 404 error  with a message if invalid api_key is received' do
        user = create(:user)
        request_params = { api_key: 'JJ-3dc0vW5if5w8PBdFqvg'}
